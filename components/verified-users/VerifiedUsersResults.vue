<template>
  <TheModal v-model="isVisible">
    ...
    <div class="results-container">
      <VerifiedUsersUser
        v-for="({ username, steemit_post, media_link, additional_information }, idx) in results"
        :key="idx"
        :username="username"
        :hivePost="steemit_post"
...
const { verifiedUsers } = storeToRefs(useMainStore())
...
watch(
  ...
  () => {
    if (verifiedUsers.value?.length) {
      const { currentResults, currentPage } = usePagination([...verifiedUsers.value], itemsPerPage)
